...
int consume_event(struct parser_state *s, yaml_event_t *event)
{
    ...
    case STATE_PROCESS:
        ...
        case YAML_SCALAR_EVENT:
            ...
            p->path = strdup(value);
            if (s->f->plist == NULL) s->f->plist = p;
            else s->f->plist->next = p;
            s->f->p = p;
    ...
    case STATE_PROCESS_ATTRIBUTE_KEY:
        ...
        case YAML_SCALAR_EVENT:
            value = (char *)event->data.scalar.value;
            if (!strcmp(value, "user")) s->state = STATE_PROCESS_USER;
            else if (!strcmp(value, "pid")) s->state = STATE_PROCESS_PID;
            else if (!strcmp(value, "ppid")) s->state = STATE_PROCESS_PPID;
            else if (!strcmp(value, "permission")) s->state = STATE_PROCESS_PERMISSION;
            else if (!strcmp(value, "cwd")) s->state = STATE_PROCESS_CWD;
            else if (!strcmp(value, "logging")) s->state = STATE_PROCESS_LOGGING;
            break;
        case YAML_MAPPING_END_EVENT:
            s->state = STATE_PROCESS;
            break;
    ...
    case STATE_LOGGING_LIST:
        ...
        case YAML_SCALAR_EVENT:
            ...
            if (s->f->p->llist == NULL) s->f->p->llist = l;
            else s->f->p->l->next = l;
            s->f->p->l = l;
            s->state = STATE_LOGGING_LIST;
            break;
